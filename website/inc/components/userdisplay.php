    ...
    ?>
        <div class = "col-lg-6">
            <div class="card text-center">
                ...
                <?php
                    // If the user is logged in and not looking at their own profile, display the option to follow another user
                    if(isset($_SESSION["userID"]) && $_GET["userID"] !== $_SESSION["userID"]) {
                ?>
                    <hr>
                    <form method="POST" action="inc/components/follow.php">
                        <input type="text" value="" hidden>
                        <button class="btn btn-lg btn-info" type="submit">Follow</button>
                    </form>
                <?php
                    }
                    // Close if statement checking for whether user is logged in
                ?>
                </div>
            </div> 
        </div>
